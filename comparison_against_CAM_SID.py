#!/usr/bin/env python
"""
Comprehensive Comparison of FLAR vs. CAM with SID Evaluation
...
For each configuration, the script:
  ...
  2. Simulates data using either a linear or non‑linear SEM.
  3. Runs FLAR (your DAG‑Boosting method) to obtain a continuous weight matrix W.
     It computes the total effect matrix T = (I - W)⁻¹ - I and extracts the estimated ATE from T[0, n_vars-1].
     It also thresholds W to recover a binary graph and computes SHD and ATE_RMSE, as well as SID.
  4. Runs CAM (from cdt) on the same data.
  5. Records runtime, SHD, SID, and ATE_RMSE for both methods.
...
# -------------------------------
# 3. FLAR (DAG‑Boosting) Implementation
# -------------------------------
...
# -------------------------------
# 4. CAM Baseline (using cdt)
# -------------------------------
import cdt
cdt.SETTINGS.rpath = "/usr/bin/Rscript"  # Update if necessary
...
# -------------------------------
# 5. Main Comparison Experiment (FLAR vs. CAM)
# -------------------------------
...
if __name__ == '__main__':
    ...
    node_settings = [20, 50, 100]    # numbers of nodes
    dag_types = ['ER', 'SF']         # DAG types: ER and Scale-Free
    ...
    for n_vars in node_settings:
        edge_prob = 2.0 / (n_vars - 1)  # for ER: expected avg degree ~2
    ...
    df_results.to_csv("comparison_flar_cam_results_SID.csv", index=False)
    df_results.to_json("comparison_flar_cam_results_SID.json", orient="records", lines=True)
    print("Results have been saved to 'comparison_flar_cam_results_SID.csv' and 'comparison_flar_cam_results_SID.json'.")
